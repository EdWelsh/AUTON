...
needs_bases = pytest.mark.skipif(
    not all(_has_tag(t) for t in ("kernel-base-v5", "kernel-base-v4", "kernel-base-v3",
                                   "kernel-base-v2",
                                   "kernel-reference-v1")),
...
def test_an_empty_service_on_the_default_base_fails_only_for_its_own_entry(tmp_path):
    tree = _extract(tmp_path, "kernel-base-v5")
...
def test_the_base_loads_the_format_the_exporter_writes():
    ...
    src = subprocess.run(["git", "-C", str(ROOT), "show",
                          "kernel-base-v5:kernels/x86_64/kernel/slm/neural/neural_backend.c"],
                         capture_output=True, text=True).stdout
    m = re.search(r"#define VERSION\s+(\d+)u", src)
    assert m and int(m.group(1)) == VERSION


def test_the_script_defaults_to_v5():
    assert 'REV="kernel-base-v5"' in SCRIPT.read_text()


@needs_bases
def test_the_default_base_passes_the_hal_gate(tmp_path):
    from hal_gate import scan

    assert scan(_extract(tmp_path, "kernel-base-v5")) == []
