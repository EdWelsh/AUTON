---
arch: x86_64
provides: [arch-x86_64]
depends_on: [arch]
optional: []
# Implements the HAL for x86-64.
---
...
**Bit 7 is PS in a PD/PDPT entry and PAT in a PT entry** (Intel SDM Vol. 3A §4.5, Tables 4-17
and 4-19). When the VMM splits a 2 MiB page into 512 PTEs it must not copy PS across, or every
split page silently changes memory type. `subsystems/mm.md` *Huge-Page Split (REQUIRED)* states
the rule, and `tests/kernel/run_vmm_test.sh` checks it.
...
## Expected Faults (REQUIRED, hardware-truth H10)

The conformance harness runs inside an image as well as on a host
(`agent/kernel_spec/conformance/README.md`), and a suite that asks "does this
encoding raise #UD?" needs to survive the answer. Two requirements:

```c
/* Arm the next fault: vector `vec` is expected, and if it arrives the handler
 * resumes at `resume_rip` instead of halting. Returns the previous armed
 * vector, so arming nests. -1 disarms. */
int  arch_expect_fault(int vec, void *resume_rip);

/* Whether the armed fault arrived, cleared by reading. */
int  arch_fault_taken(void);
```

Rules, in the order they matter:

1. **An unexpected fault still halts**, printing `[CPU] exception <vec> at <rip>`.
   Arming one vector must never turn the kernel into one that swallows faults —
   that would convert every future bug into silence.
2. The dispatcher compares the **vector number**, not the instruction. A #GP
   where #UD was armed is an unexpected fault, and halts.
3. `resume_rip` is used **once**. The armed expectation is cleared before the
   handler returns, so a second fault at the same place halts rather than
   looping forever.
4. Arming is not allowed inside an interrupt handler. Nested expectation of the
   same vector from the handler that is servicing it cannot be resumed
   correctly, and a harness that tried would hang.
5. The harness records vector, faulting RIP and whether it was expected, so a
   verdict can say *what* faulted rather than only *that* something did.

A tree without `arch_expect_fault` cannot run the in-kernel fault suite, and
`tests/kernel/run_fault_harness_test.sh` says so with exit 2 rather than
reporting zero divergences — a suite that did not run has found nothing, and
...
| Field | Source |
|---|---|
| `vendor` | `CPUID` leaf `0x0` — EBX, EDX, ECX concatenated, in that order |
| `family`, `model`, `stepping` | `CPUID` leaf `0x1` EAX, **with the extended fields folded in** |
| `brand` | `CPUID` leaves `0x80000002`-`0x80000004`, 48 bytes, often space-padded |
| `microcode_rev` | MSR `IA32_BIOS_SIGN_ID` (`0x8B`), high 32 bits |
| `board_vendor`, `board_product` | SMBIOS type 1 (System) and type 2 (Baseboard) |

### Folding the extended fields (REQUIRED)

`CPUID.1:EAX` splits family and model across base and extended fields, and
reading only the base fields misidentifies every modern part:
...
/* Intel folds the extended model for family 6 as well as 0xF; AMD only for
 * 0xF. Getting this wrong shifts every model number and every errata lookup
 * with it. */
model    = (base_family == 0xF || base_family == 6)
             ? base_model + (ext_model << 4)
             : base_model;
stepping = eax & 0xF;
```

An Alder Lake part reports base family 6, base model 7, extended model 9 — so
family 6 **model 151**, which is what Intel's Specification Update is indexed
by. Without the fold it reads as model 7, and matches a Pentium III's errata.

### Reading the microcode revision (REQUIRED SEQUENCE)

The order is architectural, not a convention, and getting it wrong returns
stale data with no error:

```c
wrmsr(IA32_BIOS_SIGN_ID, 0);     /* 1. clear the MSR */
cpuid(1, &a, &b, &c, &d);        /* 2. serialise — CPUID is the documented
                                  *    instruction for this, and the compiler
                                  *    must not elide the call */
uint64_t v = rdmsr(IA32_BIOS_SIGN_ID);
microcode_rev = v >> 32;         /* 3. revision is the HIGH dword */
```

Skipping step 1 or 2 leaves whatever the MSR held before — frequently a
plausible-looking revision from a previous read. This is the single most
dangerous field to get wrong in this specification: it is what distinguishes
"fixed in silicon" from "patched at boot" from "unpatched", and a wrong answer
to *"is this machine safe?"* is worse than no answer.

Under QEMU without `-cpu host`, the MSR commonly reads 0 after this sequence.
**0 after a correct read is still a real reading**, distinct from not having
looked: report `microcode_src = IDENT_READ` with `microcode_rev = 0`. A
consumer treats a 0 revision as "no microcode update applied", which for an
emulated CPU is true.

### SMBIOS

The entry point is found from the ACPI/EFI configuration tables, or by scanning
`0xF0000`-`0xFFFFF` for `_SM_` / `_SM3_` on legacy BIOS. Absent under QEMU's
default machine, in which case `board_src = IDENT_UNKNOWN`.
