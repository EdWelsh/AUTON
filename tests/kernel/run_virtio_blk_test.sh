#!/usr/bin/env bash
# Compile and run the VirtIO Block virtqueue tests on the host.
#
#   tests/kernel/run_virtio_blk_test.sh --self-test    # against the reference
#   KERNEL_TREE=<dir> tests/kernel/run_virtio_blk_test.sh
#
...
OUT="${TMPDIR:-/tmp}/auton_virtio_blk_test"
CC="${CC_HOST:-clang}"
...
if [ "${1:-}" = "--self-test" ]; then
	...
	"$CC" -O1 -g -fsanitize=address,undefined \
		...
		"$HERE/virtio_blk_test.c" "$HERE/virtio_reference/virtio_ref.c" \
		-o "$OUT" || exit 1
	exec "$OUT"
fi
...
if [ ! -d "$KERNEL_TREE/kernel" ]; then
	...
	echo "virtio-blk is specified in agent/kernel_spec/subsystems/drivers.md" >&2
	echo "and recorded in agent/kernel_spec/drivers/virtio-blk.md at" >&2
...
SOURCES=""
for c in kernel/drivers/blk/virtio_blk.c kernel/blk/virtio_blk.c; do
	[ -f "$KERNEL_TREE/$c" ] && SOURCES="$SOURCES $KERNEL_TREE/$c"
done
if [ -z "$SOURCES" ]; then
	echo "kernel tree present but no virtio_blk.c in it." >&2
	exit 2
fi
...
"$CC" -O1 -g -fsanitize=address,undefined \
	...
	"$HERE/virtio_blk_test.c" $SOURCES -o "$OUT" || {
