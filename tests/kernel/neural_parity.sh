...
# The kernel tree is a PARAMETER, not this script's parent. Verification must
# not live inside the artifact it verifies: the tree is agent-generated output,
# and a harness stored alongside it would be regenerated by the very code it is
# supposed to check. Point KERNEL_TREE at whatever was just built.
...
if [ ! -d "$KERNEL_TREE/kernel" ]; then
	...
	echo "Generate one first, or set KERNEL_TREE=<dir>." >&2
	exit 2
fi

HARNESS="$(cd "$(dirname "$0")" && pwd)/neural_forward_host.c"
cd "$KERNEL_TREE"
clang -O2 -Ikernel/include kernel/slm/neural/neural_backend.c kernel/lib/kmath.c \
	"$HARNESS" -lm -o /tmp/neural_forward_host || exit 1
